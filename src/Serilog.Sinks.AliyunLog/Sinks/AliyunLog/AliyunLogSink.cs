...
namespace Serilog.Sinks.AliyunLog
{
    class AliyunLogSink : PeriodicBatchingSink
    {
        ...
        async protected override Task EmitBatchAsync(IEnumerable<LogEvent> events)
        {
            ...
            if (logTags != null) logGroupInfo.LogTags = logTags;
            ...
            try
            {
                var request = new PostLogsRequest(this.logstoreName, logGroupInfo);
                var response = await logServiceClient.PostLogStoreLogsAsync(request);
